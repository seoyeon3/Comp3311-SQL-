...
--set mark to zero where grade is 'AF'
--update course_enrolments set mark = 0 where Grade = 'AF';


--update course_enrolments set mark = int(-) where mark is null;
--나중에 coursetitle로 바꾸기
...
as
--select p.code, p.name, p.uoc, p.duration, ou.name, aog.name, aog.type, aog.defby, aog.definition,r.min_req, r.max_req
...
create or replace view completed_courses(zid, CourseCode, Term, CourseTitle, Mark, Grade, UOC)
as
select s.id, sb.code, t.code ,sb.name, c_e.mark, c_e.grade, sb.uoc from students s 
...
create or replace view stream_rulename --t1
as
...
create or replace view program_rulename --t2
as
select l.zid, r.name, r.type, r.id, aog.definition, aog.defby, r.min_req, r.max_req
from  lastPgEnrol l 
...
create or replace view all_rulename --t3
as
...
as
select c.zid, c.coursecode, c.term, c.coursetitle, c.mark, c.grade,c.uoc, a.name, a.type, a.min_req, a.max_req
from completed_courses c
left join all_rulename a --t3 t
on c.zid = a.zid and a.definition ILIKE '%' || c.coursecode || '%'
order by c.zid
;

--match course with rulename which include # but not gen rule
create or replace view ruleWithHash --a1
as
select a.zid, a.coursecode, a.term, a.coursetitle, a.mark, a.grade, a.uoc, b.name, b.type, b.min_req, b.max_req
...
create or replace view WithAllRule (zid, coursecode,term,coursetitle, mark, grade, uoc, type, rule, min, max)
as
select c.zid, c.coursecode, c.term, c.coursetitle, c.mark, c.grade, c.uoc, concat(c.type, a.type, g.type), concat(c.name, a.name, g.name), concat(c.min_req, a.min_req, g.min_req), concat(c.max_req, a.max_req, g.max_req)
...
FROM WithAllRule
GROUP BY zid, coursecode, term, coursetitle, mark, grade, uoc, type
order by zid, term, coursecode
;
